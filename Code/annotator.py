...
#         p = Path(verts)
#         ind = p.contains_points(pix, radius=1)
#         selected_region = np.zeros_like(data)
#         selected_region.flat[ind] = data.flat[ind]
#         selected_mask = np.tile(False, data.shape)
#         selected_mask.flat[ind] = True
        
#         selected['data'] = selected_region
...
#         if bbox:
#             b = path_bbox(verts)
#             selected_region = selected_region[int(min(b[:, 1])):int(max(b[:, 1])),
#                                               int(min(b[:, 0])):int(max(b[:, 0]))]
#             selected['data'] = selected_region
